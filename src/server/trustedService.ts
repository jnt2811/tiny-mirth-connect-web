...
export const getTrustedServices = createServerFn().handler(async () => {
  const [rows] = await db.query<TrustedService[]>('SELECT * FROM trusted_service ORDER BY ID')
  return rows
})

export const createTrustedService = createServerFn({ method: 'POST' })
  .inputValidator((data: { SERVICE_NAME: string; SERVICE_HOST: string; STATUS: number }) => data)
  .handler(async ({ data }) => {
    await db.execute(
...
export const updateTrustedService = createServerFn({ method: 'POST' })
  .inputValidator(
    (data: { ID: number; SERVICE_NAME: string; SERVICE_HOST: string; STATUS: number }) => data,
  )
  .handler(async ({ data }) => {
    await db.execute(
...
export const updateTrustedServiceStatus = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    await db.execute('UPDATE trusted_service SET STATUS = ? WHERE ID = ?', [data.STATUS, data.ID])
  })
